...
def validate_yml_config(config: Dict) -> bool:
    ...
    if 'algorithm' not in config['vqe']['optimization'] or not isinstance(config['vqe']['optimization']['algorithm'], str):
        raise ValueError("Missing or invalid 'algorithm' in 'optimization'. It should be a string.")
    
    if 'constraint' not in config['vqe']['optimization'] or not isinstance(config['vqe']['optimization']['constraint'], bool):
