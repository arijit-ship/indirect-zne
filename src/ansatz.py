...
def create_redundant_NEW(
    ...
    """
    NEW IMPLEMENTATION OF CREATE_REDUNDANT FUNCTION!
    Review needed.
    """

    noisy_gate_count: int = 0
    chunks: List = []
    flag: int = layers  # Tracking angles in param ndarrsy
    ...
    for layer in range(layers):
        ...
        # Add Rx to first and second qubits
        circuit.add_RX_gate(0, param[flag])
        circuit.add_noise_gate(Identity(0), noise_type, noise_r_prob)  # Add depolarizing noise
        circuit.add_RX_gate(1, param[flag + 1])
        circuit.add_noise_gate(Identity(1), noise_type, noise_r_prob)  # Add depolarizing noise

        noisy_gate_count += 2  # Two RX gates added
        ...
        for _ in range(r_gate_factor):
            ...
            # First qubit
            circuit.add_RotInvX_gate(0, param[flag])
            circuit.add_noise_gate(Identity(0), noise_type, noise_r_prob)  # Add depolarizing noise
            circuit.add_RX_gate(0, param[flag])
            circuit.add_noise_gate(Identity(0), noise_type, noise_r_prob)  # Add depolarizing noise

            # Second qubit
            circuit.add_RotInvX_gate(1, param[flag + 1])
            circuit.add_noise_gate(Identity(1), noise_type, noise_r_prob)  # Add depolarizing noise
            circuit.add_RX_gate(1, param[flag + 1])
            circuit.add_noise_gate(Identity(1), noise_type, noise_r_prob)  # Add depolarizing noise

            noisy_gate_count += 4  # Two RX and two RX_dagger gates added

        # Add Ry to first and second qubits
        circuit.add_RY_gate(0, param[flag + 2])
        circuit.add_noise_gate(Identity(0), noise_type, noise_r_prob)  # Add depolarizing noise
        circuit.add_RY_gate(1, param[flag + 3])
        circuit.add_noise_gate(Identity(1), noise_type, noise_r_prob)  # Add depolarizing noise

        noisy_gate_count += 2  # Two RY gates added

        # Add identities with Ry and make redudant circuit
        for _ in range(r_gate_factor):
            # First qubit
            circuit.add_RotInvY_gate(0, param[flag + 2])  # Ry_dagger
            circuit.add_noise_gate(Identity(0), noise_type, noise_r_prob)  # Add depolarizing noise
            circuit.add_RY_gate(0, param[flag + 2])
            circuit.add_noise_gate(Identity(0), noise_type, noise_r_prob)  # Add depolarizing noise

            noisy_gate_count += 2  # Two RY and two RY_dagger gates added

            # Second qubit
            circuit.add_RotInvY_gate(1, param[flag + 3])  # Ry_dagger
            circuit.add_noise_gate(Identity(1), noise_type, noise_r_prob)  # Add depolarizing noise
            circuit.add_RY_gate(1, param[flag + 3])
            circuit.add_noise_gate(Identity(1), noise_type, noise_r_prob)  # Add depolarizing noise
        ...
        noisy_gate_count += 2  # One CZ gate added
        ...
        for _ in range(cz_gate_factor):
            ...
            noisy_gate_count += 4  # Two CZ gates added
        ...
        for i in range(nqubits):
            circuit.add_noise_gate(Identity(i), noise_type, noise_u_prob)
        noisy_gate_count += nqubits  # One identity gate for each qubit added

        # XY spin chain identity

        for _ in range(u_gate_factor):
            # Add Y gates to all odd qubits
            circuit, y_count = add_ygate_odd(circuit, noise_type, noise_y_prob, y_gate_factor)
            noisy_gate_count += y_count  # Y gates added
    ...
    output: dict = {
        "chunks": chunks,
        "circuit": circuit,
        "noisy_gate_counts": noisy_gate_count,
    }

    return output

def add_ygate_odd_NEW(circuit: QuantumCircuit, noise_type: str, noise_y_prob: float, y_gate_factor: int) -> QuantumCircuit:
    """
    Adds Y gates to odd qubit wires.
    ...
    for i in range(qubit_count):
        if (i + 1) % 2 != 0:
            ...
            circuit.add_noise_gate(Identity(i), noise_type, noise_y_prob)  # Add depolarizing noise
            # circuit.add_noise_gate(Y(i), "Depolarizing", noise_y_prob)
            count += 1

            # Add redundant Y gate identities
            for _ in range(y_gate_factor):
                circuit.add_Y_gate(i).get_inverse()
                circuit.add_noise_gate(Identity(i), noise_type, noise_y_prob)
                circuit.add_Y_gate(i)
                circuit.add_noise_gate(Identity(i), noise_type, noise_y_prob)
                count += 2
                # circuit.add_noise_gate(Y(i).get_inverse(), "Depolarizing", noise_y_prob)
...
"""
Old implementation of create_redundant function.
Two-qubit depolarization noise is fundamentally different from applying single-qubit depolarization noise to each qubit
separately.
"""
# def create_redundant(
#     nqubits: int,
